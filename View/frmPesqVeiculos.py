...
class Ui_frmPesqVeiculos(object):
    ...
    def ExcluirVeiculo_Click(self):
        ...
        msg = QMessageBox()
        msg.setWindowIcon(QIcon("Imagens/excluir.png"))
    ...
    def atualizar_lista(self, text):
        if text.isdigit():
            self.PesquisarVeiculo(text, 'AnoModelo')
        else:
            # dividir a entrada de texto em palavras separadas por espaço em branco
            # palavras = text.split()
        # if len(palavras) >= 1:
            # se houver pelo menos duas palavras, a primeira é a marca e a segunda é o modelo
            #marca = palavras[0]
            ##modelo = " ".join(palavras[1:])
            #self.PesquisarVeiculo(marca, 'Marca')
            #self.PesquisarVeiculo(modelo, 'Modelo')
        #else:
            # se houver apenas uma palavra, é a marca ou o modelo (deixa o outro campo vazio)
            self.PesquisarVeiculo(text, 'Modelo')
            #self.PesquisarVeiculo('', 'Modelo')
    ...
    def setupUi(self, frmPesqVeiculos):
        ...
        # desabilitar redimensionamento de tela
        frmPesqVeiculos.setFixedSize(820, 504)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(_fromUtf8("Imagens/btnListVeiculo.png")), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        self.edtPesquisa.setGeometry(QtCore.QRect(190, 90, 500, 20))
        self.edtPesquisa.setObjectName(_fromUtf8("edtPesquisa"))
        self.edtPesquisa.setPlaceholderText("Pesquisar TUDO")
        self.edtPesquisa.textChanged.connect(self.atualizar_lista)
    ...
    def retranslateUi(self, frmPesqVeiculos):
        ...
        item.setText(_translate("frmPesqVeiculos", "Condição", None))
        item = self.gridVeiculos.horizontalHeaderItem(7)
        item.setText(_translate("frmPesqVeiculos", "KMetragem", None))
        ...
        self.lblTotal.setText(_translate("frmPesqVeiculos", "Pesquisar/Atualizar", None))
        self.btnExcluir.setText(_translate("frmPesqVeiculos", "Excluir", None))
        self.btnAlterar.setText(_translate("frmPesqVeiculos", "Editar", None))
