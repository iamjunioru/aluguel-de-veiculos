...
class Ui_FrmPesqAluguel(object):
    def DevolverVeiculo(self):
        ...
        msg = QMessageBox()
        msg.setWindowIcon(QIcon("Imagens/confirm1.png"))
    ...
    def PesquisarTodosAluguel(self):
        ...
        # ler dados and cria dataframe
        data = []
        while query.next():
                row = {
                    'codigoAlug': str(query.value(0)),
                    'nomecliente': str(query.value(10)),
                    'dataAlug': str(query.value(1)),
                    'dataPrazo': str(query.value(2)),
                    'dataDevolucao': str(query.value(3)),
                    'valorAluguel': str(query.value(4)),
                    'valorMulta': str(query.value(5)),
                    'kmEntrada': str(query.value(6)),
                    'kmSaida': str(query.value(7))
                }
                data.append(row)
        ...
        for row in df.itertuples():
            self.gridAluguel.insertRow(row.Index)
            self.gridAluguel.setItem(row.Index, 0, QTableWidgetItem(row.codigoAlug))
            self.gridAluguel.setItem(row.Index, 1, QTableWidgetItem(row.nomecliente))
            self.gridAluguel.setItem(row.Index, 2, QTableWidgetItem(row.dataAlug))
            self.gridAluguel.setItem(row.Index, 3, QTableWidgetItem(row.dataPrazo))
            self.gridAluguel.setItem(row.Index, 4, QTableWidgetItem(row.dataDevolucao))
            self.gridAluguel.setItem(row.Index, 5, QTableWidgetItem(row.valorAluguel))
            self.gridAluguel.setItem(row.Index, 6, QTableWidgetItem(row.valorMulta))
            self.gridAluguel.setItem(row.Index, 7, QTableWidgetItem(row.kmEntrada))
            self.gridAluguel.setItem(row.Index, 8, QTableWidgetItem(row.kmSaida))
    ...
    def setupUi(self, FrmPesqAluguel):
        ...
        self.btnPesquisa.setIconSize(QtCore.QSize(20, 20))
        self.btnPesquisa.setObjectName(_fromUtf8("btnPesquisa"))
        # botão pesquisa CLICK
        ...
        self.btnDevolver.setGeometry(QtCore.QRect(394, 1, 101, 61))
        ...
        self.btnDevolver.setIconSize(QtCore.QSize(20, 20))
        ...
        self.btnExportar = QPushButton(FrmPesqAluguel)
        self.btnExportar.setGeometry(QtCore.QRect(404, 330, 101, 23))
        self.btnExportar.setObjectName(_fromUtf8("btnExportar"))
        self.btnExportar.setText("Exportar Excel")
        self.btnExportar.clicked.connect(self.exportarExcel)
        ...
        self.edtDevolucao.setPlaceholderText("dd/mm/aaaa")
        ...
        self.edtMulta.setPlaceholderText("R$ 0,00")       
        ...
        self.edtSaida.setPlaceholderText("KM de saída")
