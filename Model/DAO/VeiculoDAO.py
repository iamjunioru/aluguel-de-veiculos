...
#  tem como objetivo realizar operações no banco de dados relacionadas aos veículos.
#  essas operações incluem: cadastrar um novo veículo, atualizar informações de um veículo existente, excluir um veículo, pesquisar veículos disponíveis, pesquisar todos os veículos e pesquisar um veículo específico.
class VeiculoDAO:
    ...
    def PesquisarVeiculo(valor, tipo):
        ...
        if tipo=='Código':
            ...
            sql = "SELECT * FROM Veiculo WHERE Marca LIKE '%" + valor + "%'"
            query = QSqlQuery(sql)
        elif tipo == 'Modelo':
            sql = "SELECT * FROM Veiculo WHERE Modelo LIKE '%" + valor + "%'"
            query = QSqlQuery(sql)
        elif tipo == 'AnoModelo':
            sql = "SELECT * FROM Veiculo WHERE AnoModelo LIKE '%" + valor + "%'"
            query = QSqlQuery(sql)
